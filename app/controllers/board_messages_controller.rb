class BoardMessagesController < ApplicationController
  ...
  def index
    ...
    @board_user = users
    @board_messages = BoardMessage.new
    @board_messages.board_user_id = @board_user.id
  end
  ...
  def create
    ...
    @board_user = BoardMessage.where('account_id == ?', current_account.id)[0]
    respond_to do |format|
      if @board_message.save
        format.html { redirect_to '/board_messages'}
        format.json { render :show, status: :created, location: @board_messages }
      else
